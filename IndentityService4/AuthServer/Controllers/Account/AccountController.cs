...
namespace IdentityServer4.Quickstart.UI
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginInputModel model, string button)
        {
            ...
            {
                // 验证内存存储中的用户名/密码
                var user = new User { Name = model.Username, Id = Convert.ToInt32(model.Password) }; //SqlSugarDbContext.Instance.ExecutedSql().Queryable<User>().Where(w => w.Phone == model.Username).First();
                if (user != null)
                {
